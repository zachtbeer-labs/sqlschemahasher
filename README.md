...
{
    NormalizeAutoGeneratedIndexNames = true,  // Neutralize auto-generated index name suffixes
...
| `IgnoreIndexNames` | `false` | `false` | `false` | `true` |
| `NormalizeClusteringType` | `false` | `false` | `false` | `true` |
| `IgnoreIndexSortOrder` | `false` | `true` | `false` | `true` |
| `NormalizeAutoGeneratedIndexNames` | `false` | `false` | `false` | `false` (moot — names ignored) |
| `IncludeStoredProcedureText` | `true` | `true` | `true` | `true` |
...
| `NormalizeAutoGeneratedIndexNames` | `false` | Neutralizes auto-generated index names (SQL Server system names like `PK__Table__3214EC07A1B2C3D4` and GUID-suffixed names like `nci_wi_Table_<32 hex chars>`), so suffixes minted differently per database don't affect the hash. Explicit names still compare exactly. |
| `IgnoreIndexNames` | `false` | Excludes index names from the hash entirely — only the index definition matters. Takes precedence over `NormalizeAutoGeneratedIndexNames`. |
| `NormalizeClusteringType` | `false` | Normalizes `clustered`/`nonclustered` to a common value |
| `IgnoreIndexSortOrder` | `false` | Excludes index key sort order (ASC/DESC) from the hash |
